...
namespace Kaleidoscope.Models
{
    ...
    public class PlcMasterConfig
    {
        ...
        /// </summary>
        [DisplayName("重连节流间隔(ms)")]
        [Description("连接断开后两次重连尝试至少间隔这么久，默认 5000 防高频无效连接")]
        ...
        /// </summary>
        [DisplayName("轮询周期(ms)")]
        [Description(">0 且轮询项非空时自动后台轮询；0=不自动轮询（业务层按需读写）")]
        public int PollIntervalMs { get; set; } = 1000;

        /// <summary>自动轮询项列表（每条一个名称+功能码+起始地址+数量），见 <see cref="PlcPollItem"/>。</summary>
        [DisplayName("自动轮询项")]
        [Description("自动轮询项列表（双击展开编辑）：每条 = 名称 + 功能码 + 起始地址 + 数量")]
